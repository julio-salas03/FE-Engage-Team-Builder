...
const downloadImage = async (url, base_image_path) => {
  ...
  const saveImage = async (format) => {
    ...
    await sharp(imageBuffer).toFile(outputPath, (err, info) => {
      if (err) throw err;
    });
  };

  await Promise.all([saveImage("png"), saveImage("avif"), saveImage("webp")]);
};

const buildNumericStat = (stat) => {
  if (typeof stat !== "string")
    throw new Error(
      `buildNumericStat expects stat parameter to be a string. ${typeof stat} provided`,
    );
...
const init = async () => {
  ...
  await axios
    .get("https://serenesforest.net/engage/miscellaneous/skills/")
    .then((html) => {
      ...
      for (let index = 1; index <= 4; index++) {
        ...
        $(`table:eq(${realIndex})`)
          ...
          .each((_, el) => {
            ...
            const imgPath = `${imgBasePath}/skills/${
              isInheritable ? "inheritable" : "syncho"
            }/${imgName}`;
            ...
            if (isInheritable)
              inheritableSkills.push({
                name: data[0],
                description: data[1],
                spCost: Number(data[2]),
                bondLvl: Number(bond[3]),
                emblem: bond[0],
      ...
      for (
        let index = personalSkillsStartingIndex;
        index <= personalSkillsStartingIndex + 1;
        index++
      ) {
        const realIndex =
          index === personalSkillsStartingIndex
            ? personalSkillsStartingIndex
            : personalSkillsStartingIndex + 6;
        $(`table:eq(${realIndex})`)
          .find("tr:not(:first)")
          .each((_, el) => {
            const { data, img, imgName } = extractBasicData($(el));
      ...
      for (
        let index = classSkillsStartingIndex;
        index <= classSkillsStartingIndex + 1;
        index++
      ) {
        const realIndex =
          index === classSkillsStartingIndex
            ? classSkillsStartingIndex
            : classSkillsStartingIndex + 6;
        $(`table:eq(${realIndex})`)
          .find("tr:not(:first)")
          .each((_, el) => {
            const { data, img, imgName } = extractBasicData($(el));
  ...
  for (let i = 0; i < characterNames.length; i++) {
    ...
    try {
      const request = await axios.get(
        `https://www.fireemblemwod.com/fe17/personajes/${
          character === "Alear" ? "Avatar" : character
        }.htm`,
      );
      ...
      const [bases, growths, modifiers] = $("table.w1acb0:eq(3) tr.bbs4")
        .map((_, el) => {
          const data = $(el)
            .text()
            .trim()
      ...
      const initialSP = buildNumericStat(
        $("td.separador:last").text().match(/\d+/g)?.[1],
      );

      const capability = (() => {
        const data = $("td.separador:last").text();
        const splitString = data.split("Aptitud oculta");
        const formattedCapability = splitString[0]
          .replace("Aptitud: ", "")
          .trim();
        return formattedCapability in basicWeaponsSpanish
          ? basicWeaponsSpanish[formattedCapability]
          : null;
      })();
      ...
      const hiddenCapabilities = (() => {
        ...
        if (!formattedCapabilities.includes(","))
          return [basicWeaponsSpanish[formattedCapabilities]];
        return formattedCapabilities
          .split(",")
          .map((capability) => capability.trim())
          .map((capability) =>
            capability in basicWeaponsSpanish
              ? basicWeaponsSpanish[capability]
              : null,
          );
      })();
      const { character: _, ...personalSkill } = personalSkills.find(
        (skill) => normalizeName(skill.character) === character,
      );
      charactersData.push({
        bases,
        growths,
  ...
  await axios
    .get("https://serenesforest.net/engage/somniel/forging/")
    .then((html) => {
      const $ = cheerio.load(html.data);
      $("table tr").each((_, el) => {
        ...
        weaponsUpgrades.push({
          ...
          materials: materials.reduce((acc, curr) => {
            ...
            return {
              ...acc,
              [material.toLowerCase()]: Number(amount.replace("x", "")),
            };
          }, {}),
  ...
  for (const weaponType of weaponTypes) {
    await axios
      .get(`https://serenesforest.net/engage/weapons-items/${weaponType}/`)
      .then((html) => {
        const $ = cheerio.load(html.data);
        $("table tr").each((_, el) => {
          const { data, img, imgName } = extractBasicData($(el));
          ...
          if (
            data[8].includes("wielded by Emblem") ||
            data[8].includes("of Emblem")
          )
            return allEngageWeapons.push(weapon);
          const weaponUpgrades = weaponsUpgrades.filter(
            (upgrade) => upgrade.weaponName === data[0],
          );

          weapons.push({
            ...weapon,
            base64ID: base64ID.length < 2 ? "A" + base64ID : base64ID,
            weaponUpgrades: weaponUpgrades.map(
              ({ weaponName, upgradeLvl, ...upgradeData }) => upgradeData,
            ),
          });
        });
      });
  ...
  for (let index = 0; index < emblemNames.length; index++) {
    ...
    const request = await axios.get(
      `https://serenesforest.net/engage/emblems/${emblem.toLowerCase()}/`,
    );
    const $ = cheerio.load(request.data);
    ...
    const statsModifiersNames = $("table:first tr th:not(:first)")
      .toArray()
      .map((el) => $(el).text().toLowerCase().replace("\n", ""));
    const statsModifiers = $("table:first tr:last td:not(:first)")
      .toArray()
      .map((el) => Number($(el).text()));
    const fullModifiers = statsModifiersNames.map((statName, i) => [
      statName,
      statsModifiers[i],
    ]);
    ...
    $("table:last tr").each((_, el) => {
      ...
      if (data[0].includes("Prof"))
        return weaponProficiencies.push(data[0].toLowerCase().split(" ")[0]);

      if (data[2].includes("Engage Attack")) {
        const attackPartner = data[2].includes("adjacent")
          ? data[2].split(" ").slice(-1)[0].replace(")", "")
          : null;
        const [description, ...unitTypeModifiers] =
          data[1].split(/(?=\[[^\]]+\])/);
        return attackPartner
          ...
          : (engageAttack.base = {
              name: data[0],
              description,
              unitTypeModifiers: unitTypeModifiers.map(
                createEngageAttackUnitTypeModifier,
              ),
            });
      }

      if (data[2] === "Engage Weapon") {
        const foundWeapon = allEngageWeapons.find(
          (weapon) => weapon.name === data[0],
        );
    ...
    const img = $("img")
      ...
      .toArray()
      .filter((url) => url.includes(emblem.toLocaleLowerCase()))[0];
  ...
  await axios
    .get("https://serenesforest.net/engage/somniel/engraving/")
    .then((html) => {
      const $ = cheerio.load(html.data);
  ...
  fs.writeFile(
    "./data/engravings.json",
    JSON.stringify(engravings),
    () => null,
  );
  fs.writeFile(
    "./data/class-skills.json",
    JSON.stringify(classSkills),
    () => null,
  );
  fs.writeFile(
    "./data/inheritable-skills.json",
    JSON.stringify(inheritableSkills),
    () => null,
  );
  fs.writeFile(
    "./data/syncho-skills.json",
    JSON.stringify(synchoSkills),
    () => null,
  );
};
